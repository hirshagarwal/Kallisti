...
self_location = init_location = [0, 0] # (x, y)
orientation = 0
...
def updateLocation(distance):
    if orientation == 0: # Front
        self_location[1] += distance
    elif orientation == 1: # Right
        self_location[0] += distance
...
def getSingleDistanceHelper(sensor):
    distances = [sensor.distance_centimeters for _ in range(20)]
    while True:
        ...
        if pvariance(distances) > 1:
            ...
            most_common_value = mode(distances)
            ...
            if data_map[most_common_value] >= 13:
...
def pathLoop(location, init_orientation=0):
    self_location = init_location = location
    ...
    while not_start or not same_location(self_location, init_location):
        not_start = False
        direction, distance = wallLoop(left_init)
