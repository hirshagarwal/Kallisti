...
import getch
from statistics import mean, pvariance, mode, median, pstdev
...
def roundList(inputList):
    return [round(i, 2) for i in inputList]
...
def getSingleDistanceHelper(sensor):
    ...
    # distances = list()
    # for i in range(10):
    #     distances.append(sensor.distance_centimeters)
    #     time.sleep(.1)
    # flag = 0
    # while True:
    #     if flag>5:
    ...
    #         stdev = pstdev(distances)
    #         updatedList = list()
    #         for i in range(0, len(distances)-1):
    #             if abs(distances[i]-center) <= stdev:
    #                 updatedList.append(distances[i])
    #         distances = updatedList
    #     else:
    #         if(sensor == frontSensor):
    #             print(roundList(distances))
